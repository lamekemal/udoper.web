...
new class extends Component
{
    ...
    public function savePayment(): void
    {
        ...
        $breeder->dues()->create([
            ...
            'amount' => $this->amount,
            'payment_date' => $paymentDate->toDateString(),
            'valid_until' => BreederDue::calculateValidUntil(BreederDue::TYPE_SUBSCRIPTION, $paymentDate)->toDateString(),
        ]);
...
<div>
    <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between mb-4">
        <div>
            <h1 class="text-2xl font-bold">{{ __('Gestion des Cotisations') }}</h1>
            <p class="text-sm text-slate-500">{{ __('Suivez les cotisations annuelles des éleveurs et marquez les paiements.') }}</p>
        </div>
